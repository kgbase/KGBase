...
-------------------------------------------------------------------------
ods-kg.php - modified library to read and write ods files from php
Some modifications of the library made by Konstantin Grebennikov (kgrebennikov@gmail.com)
for KGBase project (http://kgbase.ru) - add some methods of the class and some external functions) 
- see indications in the comments
This class allows in addition to original:
- parse&save ODS without using shell commands - only with ZipArchive extension
- create&read sheets with specific names
Simple example for testing this version of the class:
-------------------------------------------------------------------------
Note:
The ODS files created by this library, do not works completely correctly.
Apache Open Office 4 correctly opens such files, but  trying to save the file, 
you'll the message that saving is impossible. However, if press "Save" again - 
the file will be saved and will works correctly in the future.
Also, files created in Apache Open Office 4 can not be read correctly by the library - 
will be infinite cycle and "memory exhausted", only it's "own" files will be read correctly.
The reason of this bug:
Too many years the ods-php library was neglected :(
Modern Open Office applications uses standard OpenDocument 1.2,
so the structure of ODS documents become different. But ods-php create files according srandard 1.0
I see 2 way to solve the problem:
- use old legacy versions of OpenOffice. Easy, but bad way.
- rewrite ods-php according new standard. Good way, but someone must do it.
I'll try later, may be.
Also use created by the library files only for keeping and reading data possible, surely.
Konstantin A. Grebennikov, 06 August 2015.
*/
class ods {
    var $fonts;
    var $styles;
    ...
    function startElement($parser, $tagName, $attrs) {
        //the function is modified by K. Grebennikov:
        //$this->currentSheet change to new class property $this->currentSheetName
        //So, original names of tables in ODS file returns as class property
        //KG
        if(isset($attrs['TABLE:NAME'])){
         $this->currentSheetName = $attrs['TABLE:NAME'];
        }else{}
        //
        ...
        if($cTagName == 'style:font-face') {
            ...
            $this->sheets[$this->currentSheetName]['rows'][$this->currentRow][$this->currentCell]['attrs'] = $attrs;
            if(isset($attrs['TABLE:NUMBER-COLUMNS-REPEATED'])) {
                $times = intval($attrs['TABLE:NUMBER-COLUMNS-REPEATED']);
                $times--;
                for($i=1;$i<=$times;$i++) {
                    $cnum = $this->currentCell+$i;
                    $this->sheets[$this->currentSheetName]['rows'][$this->currentRow][$cnum]['attrs'] = $attrs;
                }
            ...
            if(isset($this->lastRowAtt['TABLE:NUMBER-ROWS-REPEATED'])) {
                ...
                for($i=1;$i<=$times;$i++) {
                    ...
                    $this->sheets[$this->currentSheetName]['rows'][$cnum][$i-1]['attrs'] = $attrs;
                }
    ...
    function characterData($parser, $data) {
        if($this->lastElement == 'table:table-cell') {
            $this->sheets[$this->currentSheetName]['rows'][$this->currentRow][$this->currentCell]['value'] = $data;
            if($this->repeat > 0) {
                for($i=0;$i<$this->repeat;$i++) {
                    $cnum = $this->currentCell - ($i+1);
                    $this->sheets[$this->currentSheetName]['rows'][$this->currentRow][$cnum]['value'] = $data;
                }
            }
        }
    }
...
<manifest:manifest xmlns:manifest="urn:oasis:names:tc:opendocument:xmlns:manifest:1.0">
 ...
 <manifest:file-entry manifest:media-type="text/xml" manifest:full-path="settings.xml"/>
    ...
    //function: add new empty sheet to ods (added by K.Grebennikov)
    function addSheet ($sheet) {
        $this->sheets[$sheet] = array();
    }
}
...
//=========================================================================================
//new functions added by K.Grebennikov
//=========================================================================
...
//==========================================================================================
//save ods file as zip (without shell_exec)
function saveOdsZip($obj,$dir,$file) {
    $path=$dir.'/'.$file;
    $charset = ini_get('default_charset');
    ini_set('default_charset', 'UTF-8');
    if(is_dir($path)){
    ...
    mkdir($path);
    file_put_contents($path.'/content.xml',$obj->array2ods());
    file_put_contents($path.'/mimetype','application/vnd.oasis.opendocument.spreadsheet');
    file_put_contents($path.'/meta.xml',$obj->getMeta('es-ES'));
    file_put_contents($path.'/styles.xml',$obj->getStyle());
    file_put_contents($path.'/settings.xml',$obj->getSettings());
    mkdir($path.'/META-INF/');
    ...
    file_put_contents($path.'/META-INF/manifest.xml',$obj->getManifest());
    //delete ods file "$dir.$file.'.ods'" if it already exists
    if(file_exists($dir.$file.'.ods')){unlink($dir.$file.'.ods');}
    else{}
    //create zip as ods
    $zip=new ZipArchive;
    if ($zip->open($path.'.ods', ZipArchive::CREATE) == true){
        $zip->addFile($path.'/content.xml','content.xml');
        $zip->addFile($path.'/mimetype','mimetype');
        $zip->addFile($path.'/meta.xml','meta.xml');
        $zip->addFile($path.'/styles.xml','styles.xml');
        $zip->addFile($path.'/settings.xml','settings.xml');
        ...
        $zip->addFile($path.'/META-INF/manifest.xml','META-INF/manifest.xml');
        $zip->close();
    }else{
        echo 'unable to create ods file!';}
    DelRec($path);
}
//==========================================================================================
//parse ods file as zip (without shell_exec)
