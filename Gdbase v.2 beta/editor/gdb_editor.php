 ...
 if($c_user['role']=='admin' || $c_user['role']=='editor'){
  ...
  if(!isset($do) && isset($dir) && isset($item)){
   ...
   foreach($list_obj as $nomen=>$obj){
    ...
    <input type="submit" name="submit" value="Ошибка (запрос на удаление)" /></form>');
    //form end
   }
  ...
  if(isset($do) && isset($dir) && isset($item)){
   ...
   if($do=='del'){
    ...
    if(!isset($object)){
     print('<p>Удалять изображения может только Администратор!</p>');
   ...
   $file_prev=$db.$itdir.'prev/'.$filename;
   $objects = $img->objects;
   print('<p><b>Вы хотите сообщить Администратору об ошибочном указании объекта на изображении:</b></p>');
   ...
   print('<br><i>Название</i>: '.$name.'<br>
     ...
     <i>Добавлено</i>: '.$datetime.'<br>
     <i>Автором</i>: '.$detauth.'<br>');
   print('<p><form enctype="multipart/form-data" action="gdb_editor.php?dir='.$dir.'&item='.$filename.'" method="post">
   <input name="do" type="hidden" value="del" />
   <input name="confirm" type="hidden" value="yes" />
   <input name="object" type="hidden" value="'.$name.'" />
   Комментарий (не обязательно)<br>
   <textarea rows="5" cols="45" name="errortext"></textarea><br>
   <input type="submit" name="submit" value="Сообщить" /></form><br>');
   print('<br><b>Нет!<br><a href="gdb_editor.php?dir='.$dir.'&item='.$filename.'">вернуться к изображению</a><br><hr><br></p>');
     //-delete object
     }else{
      $errfile = new SimpleXMLElement('<?xml version="1.0" encoding="UTF-8" standalone="yes"?><error></error>');
      $err_content = $errfile->addChild('content','&#xA;');
      $err_content->addChild('dir',$dir);
      $err_content->addChild('filename',$item);
      $err_content->addChild('name',$object);
      $err_content->addChild('requestor',$c_user['name']);
      $err_content->addChild('comment',$errortext);
      $err_content->addChild('time',$err_time);
      $errfile->asXML($errlog.$err_time.'.xml');
      print('Запрос на удаление объекта с именем <b>'.$object.'</b>, связанного с изображением <b>'.$item.'</b> в директории <b>'.$dir.'</b> успешно отправлен Администраторам для принятия окончательного решения.<br>');
