...
//function: add new empty sheet to file (added by K.Grebennikov)
...
//function: save ods file as zip (added by K.Grebennikov)
function saveOdsZip($dir,$file) {
    $charset = ini_get('default_charset');
    ini_set('default_charset', 'UTF-8');
    @mkdir($dir);
    file_put_contents($dir.'/content.xml',$object->array2ods());
    file_put_contents($dir.'/mimetype','application/vnd.oasis.opendocument.spreadsheet');
    file_put_contents($dir.'/meta.xml',$object->getMeta('es-ES'));
    file_put_contents($dir.'/styles.xml',$object->getStyle());
    file_put_contents($dir.'/settings.xml',$object->getSettings());
    @mkdir($dir.'/META-INF/');
    @mkdir($dir.'/Configurations2/');
    @mkdir($dir.'/Configurations2/acceleator/');
    @mkdir($dir.'/Configurations2/images/');
    @mkdir($dir.'/Configurations2/popupmenu/');
    @mkdir($dir.'/Configurations2/statusbar/');
    @mkdir($dir.'/Configurations2/floater/');
    @mkdir($dir.'/Configurations2/menubar/');
    @mkdir($dir.'/Configurations2/progressbar/');
    @mkdir($dir.'/Configurations2/toolbar/');
    file_put_contents($dir.'/META-INF/manifest.xml',$object->getManifest());
    //delete ods file "$dir.$file.'.ods'" if it already exists
    if(file_exists($dir.$file.'.ods')){unlink($dir.$file.'.ods');}
    else{}
    //create zip as ods
    $zip=new ZipArchive;
    if ($zip->open($dir.$file.'.ods', ZipArchive::CREATE) === true){
    ...
    unlink($dir.'META-INF/manifest.xml');
    rmdir($dir.'/META-INF/');
    rmdir($dir.'/Configurations2/acceleator/');
    rmdir($dir.'/Configurations2/images/');
    rmdir($dir.'/Configurations2/popupmenu/');
    rmdir($dir.'/Configurations2/statusbar/');
    rmdir($dir.'/Configurations2/floater/');
    rmdir($dir.'/Configurations2/progressbar/');
    rmdir($dir.'/Configurations2/toolbar/');
    rmdir($dir.'/Configurations2/menubar/');
    rmdir($dir.'/Configurations2/');
    unlink($dir.'/content.xml');
    unlink($dir.'/mimetype');
    unlink($dir.'/meta.xml');
    unlink($dir.'/styles.xml');
    unlink($dir.'/settings.xml');
}
